...
class WynncraftRequest(RequestBase):
    ...
    _wynncraft_rl: Ratelimit = Ratelimit(300, 60)
    ...
    # Online Players
    async def get_online_uuids_response(self) -> ReturnGather:
        url_parameters: str = "/player?identifier=uuid"
        return await self.get(url_parameters, WynncraftRequest._wynncraft_rl)

    async def get_online_uuids(self) -> Union[BaseException, "OnlinePlayerList"]:
        response: ReturnGather = await self.get_online_uuids_response()
        return await response.json() if isinstance(response, ClientResponse) else response
