...
class FetchPlayer:
    ...
    async def run(self) -> None:
        ...
        if self._fetch_queue:
            ...
            await self._to_db()
            asyncio.create_task(
                VindicatorWebhook.send(
                    "fetch_player",
                    "success",
                    f"**latest_fetched**={len(self._fetched_players)}:t={perf_counter()-t0:.2f} "
                )
            )
    ...
    def _update_fetch_queue(self) -> None:
        ...
        ---------
            - `self._fetch_queue`
        """
        for uuid in FetchOnline._logons.copy():  # Adds logged on into queue (if not already)
    ...
    async def _to_db(self) -> None:
        ...
        async with asyncio.TaskGroup() as tg:
            tg.create_task(PlayerMainInfo.to_db(self._fetched_players))
            tg.create_task(PlayerCharacterInfo.to_db(self._fetched_players))
            tg.create_task(PlayerMain.to_db(self._fetched_players))
            tg.create_task(PlayerCharacter.to_db(self._fetched_players))
