...
from kans.db.model import GuildHistory, GuildHistoryId
...
class TestGuildHistoryRepository(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_exists(self) -> None:
        ...
        for e in self._test_data:
            ...
            res = await self._repo.exists(GuildHistoryId(e.name, e.datetime))
            exists.append(res)
    ...
    async def test_find_one(self) -> None:
        ...
        test_name = {e.name for e in self._test_data}
    ...
    async def test_find_all(self) -> None:
        ...
        # ASSERT
        found_name = {e.name for e in found}
        test_name = {e.name for e in self._test_data}        # NOTE: Assert if the number of found entities is the same as the inserted entities
        self.assertEquals(len(self._test_data), len(found))
        # NOTE: Assert if the found entities are the same as the inserted entities
        self.assertSetEqual(found_name, test_name)

    @unittest.skip("Update method is not implemented in GuildHistoryRepository")
    ...
    async def test_delete(self) -> None:
        ...
        # ACT
        n = await self._repo.delete(GuildHistoryId(self._test_data[0].name, self._test_data[0].datetime))
